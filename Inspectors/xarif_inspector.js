...
function inspectSelection()
{
    ...
    var conditionText = findObject("condition");
    var inlineObj = findObject("inline");
    ...
    if(conditionText) {
        if (theObj.getAttribute("condition"))
            conditionText.value=theObj.getAttribute("condition");
        else
            conditionText.value="";
    }

    if(inlineObj) {
        if (theObj.getAttribute("inline") != "")
            selectMenuItem(inlineObj,theObj.getAttribute("inline"));
        else
            inlineObj.selectedIndex=0;
    }
...
function updateTag(attrib)
{
    ...
    if (conditionText && conditionText.value != "") {
        theObj.setAttribute("condition",conditionText.value);
    } else {
        theObj.removeAttribute("condition");
    }

    if (inlineObj && inlineObj.selectedIndex > 0) {
        theObj.setAttribute("inline",inlineObj.options[inlineObj.selectedIndex].value);
    } else {
        theObj.removeAttribute("inline");
    }
